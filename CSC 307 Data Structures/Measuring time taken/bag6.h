...
namespace main_savitch_10
{
	...
	typename bag<Item>::size_type bst_remove_all
		(binary_tree_node<Item>*& root_ptr, binary_tree_node<Item>* prev_ptr, const Item& target)
		// Precondition: root_ptr is a root pointer of a binary search tree 
		// or may be NULL for an empty tree).
		...
		// of the target were removed.
	{
		...
		*
		...
		binary_tree_node<Item> *oldroot_ptr;
		typename bag<Item>::size_type deleted = 0;
		if (root_ptr == NULL)
		{   // Empty tree
			return 0;
		}
		...
		{   // Continue looking in the left subtree
			...
			prev_ptr = root_ptr;
			root_ptr = root_ptr->left();
			deleted += bst_remove_all(root_ptr, prev_ptr, target);
			return deleted;
		}
		...
		bst_remove_all(root_ptr, prev_ptr, target);
	...
	*/
		...
		// of the target were removed.
	{
		...
		*/
		...
		if (target < root_ptr->data())
		{   // Continue looking in the left subtree
			deleted += bst_remove_all(root_ptr->left(), target);
			return deleted;
		}

		if (target > root_ptr->data())
		{   // Continue looking in the right subtree
			deleted += bst_remove_all(root_ptr->right(), target);
			return deleted;
		}
	...
	typename bag<Item>::size_type bag<Item>::count(const Item& target) const
	{
		...
		/* STUDENT WORK */
		//search starting at root node
		cursor = root_ptr;
		while (cursor != NULL)
		{
			//if target less than node, continue left
			if (target < cursor->data())
			{
				cursor = cursor->left();
			}
			//if target greater than node, continue right
			else if (target > cursor->data())
			{
				cursor = cursor->right();
			}
			//if number is equal to target, increment count and continue to the left
			else if (target == cursor->data())
			{
				answer += 1;
				cursor = cursor->left();
	...
	{
		//binary_tree_node<Item> *prev_ptr = root_ptr;
