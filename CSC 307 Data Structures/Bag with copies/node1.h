...
namespace main_savitch_5
{
	...
	{ 
		data_field = new std::pair<value_type, int>(init_data, 1);
		link_field = init_link; 
	}

	// Member functions to set the data and link fields:
    	void set_data(const value_type& new_data) { data_field->first = new_data; }
		void increment_count() { data_field->second ++; }
		void decrement_count() { data_field->second--; }
    	void set_link(node* new_link)             { link_field = new_link; }

	// Constant member function to retrieve the current data:
	value_type data( ) const { return data_field->first; }
	int count() const { return data_field->second; }
    ...
    private:
		std:: pair<value_type, int> *data_field;
	node* link_field;
    };
